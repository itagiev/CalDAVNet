...
public class CalDAVClient
{
    ...
    public async Task<MultistatusResponse> SendForMultistatusResponseAsync(HttpRequestMessage request,
    ...
    public async Task<SimpleResponse> SendForSimpleResponseAsync(HttpRequestMessage request, CancellationToken cancellationToken = default)
    {
        var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false);
        return new SimpleResponse((int)response.StatusCode);
    }
    ...
    {
        var request = new HttpRequestMessage(Propfind, "")
        {
        ...
        var response = await SendForMultistatusResponseAsync(request, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<Principal?> GetPrincipalAsync(string upn, CancellationToken cancellationToken = default)
    {
        ...
        var request = new HttpRequestMessage(Propfind, upn)
            .WithDepth(0)
            .WithPrefer("return-minimal");
    ...
    public async Task<List<Calendar>> GetCalendarsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var request = new HttpRequestMessage(Propfind, href)
        {
    ...
    {
        var request = new HttpRequestMessage(Mkcalendar, $"{href}events/")
        {
            Content = body.ToStringContent()
        }
        .WithPrefer("return=representation");

        var response = await SendForSimpleResponseAsync(request, cancellationToken).ConfigureAwait(false);
    ...
    {
        var request = new HttpRequestMessage(Proppatch, href)
        {
            Content = body.ToStringContent()
        }
        .WithVersion(ctag)
    ...
    public async Task<List<Event>> GetEventsAsync(string href, XDocument body,
        CancellationToken cancellationToken = default)
    {
        var request = new HttpRequestMessage(Report, href)
        {
            Content = body.ToStringContent()
        }
        .WithDepth(0)
    ...
    /// <summary>
    /// Gets a single event by multiget body <see cref="BodyHelper.CalendarMultiget(string[])"/>.
    ...
    public Task<Event?> GetEventAsync(string calendarHref, string eventHref,
        CancellationToken cancellationToken = default)
        => GetEventAsync(calendarHref, BodyHelper.CalendarMultiget(eventHref));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await SendForSimpleResponseAsync(request, cancellationToken);
        return response.IsSuccessStatusCode;
    }
